...
public class AbstractSvgVisitor extends PrettyPrintVisitor {
	
	public AbstractSvgVisitor(PrettyPrinterConfiguration prettyPrinterConfiguration) {
	...
	private void printModifiers(final EnumSet<Modifier> modifiers) {
		if (modifiers.size() > 0) {
			this.printer.print(modifiers.stream()
					.map(Modifier::asString)
	...
	private void printTypeArgs(final NodeWithTypeArguments<?> nodeWithTypeArguments, final Void arg) {
		NodeList<Type> typeArguments = (NodeList) nodeWithTypeArguments.getTypeArguments().orElse(null);
	...
	private void printPrePostFixOptionalList(final NodeList<? extends Visitable> args, final Void arg, String prefix,
	...
	private void printPrePostFixRequiredList(final NodeList<? extends Visitable> args, final Void arg, String prefix,
											 String separator, String postfix) {
		this.printer.print(prefix);
		if (!args.isEmpty()) {
			Iterator i = args.iterator();
	...
	public void visit(final JavadocComment n, final Void arg) {
		if (this.configuration.isPrintComments() && this.configuration.isPrintJavadoc()) {
			this.printer.println("/**");
			String commentContent = Utils.normalizeEolInTextBlock(n.getContent(),
					this.configuration.getEndOfLineCharacter());
	...
	public void visit(final ClassOrInterfaceType n, final Void arg) {
		...
		Iterator<AnnotationExpr> var3 = n.getAnnotations().iterator();
		
		
		while (var3.hasNext()) {
			AnnotationExpr ae = var3.next();
			ae.accept(this, arg);
			this.printer.print(" ");
		}
	...
	public void visit(final TypeParameter n, final Void arg) {
		...
		while (i.hasNext()) {
			AnnotationExpr ann = (AnnotationExpr) i.next();
			ann.accept(this, arg);
			this.printer.print(" ");
		}
	...
	public void visit(final ArrayType n, final Void arg) {
		...
		while (var7.hasNext()) {
			ArrayType arrayType_ = (ArrayType) var7.next();
			this.printAnnotations(arrayType_.getAnnotations(), true, arg);
			this.printer.print("[]");
		}
	...
	public void visit(final VariableDeclarator n, final Void arg) {
		this.printComment(n.getComment(), arg);
		n.getName().accept(this, arg);
		n.getAncestorOfType(NodeWithVariables.class)
				.ifPresent((ancestor) -> {
	...
	public void visit(final EnumDeclaration n, final Void arg) {
		...
		if (n.getEntries().isNonEmpty()) {
			boolean alignVertically =
					n.getEntries().size() > this.configuration.getMaxEnumConstantsToAlignHorizontally() || n.getEntries().stream().anyMatch((ex) -> {
	...
	public void visit(ModuleProvidesStmt n, Void arg) {
		...
		n.getType().accept(this, arg);
		this.printPrePostFixRequiredList(n.getWithTypes(), arg, " with ", ", ", "");
		this.printer.println(";");
	}
						...
						for (i = positionOfPreviousChild + 1; i < positionOfTheChild; ++i) {
							...
							if (!(nodeToPrint instanceof Comment)) {
								throw new RuntimeException("Expected comment, instead " + nodeToPrint.getClass() +
										"." +
										" " +
										"Position of previous child: " + positionOfPreviousChild + ", position of " +
										"child " + positionOfTheChild);
							}
